...
class ShoppingCart {
  constructor() {
    ...
    this.syncEndpoint = "/api/sync_cart";
  ...
  setupToastDefaults() {
    window.toast = window.toast || {
      ...
      error: (msg, duration = 4000) => this.showToast(msg, "error", duration),
      warning: (msg, duration = 3500) =>
        this.showToast(msg, "warning", duration),
      info: (msg, duration = 3000) => this.showToast(msg, "info", duration),
    };
  }
  ...
  async loadCartData() {
    if (window.userId) {
      try {
        const response = await fetch("/api/get_cart_data");
        const data = await response.json();
  ...
  const productId = button.dataset.productId;
    ...
    this.activeButtons.set(button, { html: originalHTML, classes: originalClasses });
  ...
  updateModalCounters() {
    ...
    if (productText) {
      productText.textContent = this.cache.uniqueCount === 1 ? "producto único" : "productos únicos";
    }
  }
    ...
    container.innerHTML = this.cartItems
      ...
      <div class="group p-4 hover:bg-gray-50 transition-colors" data-item-id="${
        ...
        <div class="flex gap-4">
          <div class="relative">
            ${
              item.product.imagen_url && item.product.imagen_url.trim() !== '' && !item.product.imagen_url.includes('default')
                ? `<img 
