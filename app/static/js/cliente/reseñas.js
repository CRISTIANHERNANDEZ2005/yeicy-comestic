      ...
      if (data.success && data.reviews.length > 0) {
        ...
        if (track) {
          ...
          data.reviews.forEach((review) => {
            ...
            // Actualizar avatar o iniciales del usuario
            const userAvatar = card.querySelector(".user-avatar");
            const userInitials = card.querySelector(".user-initials");

            if (review.usuario.avatar_url && userAvatar) {
              // Si hay avatar, mostrar imagen y ocultar iniciales
              userAvatar.src = review.usuario.avatar_url;
              userAvatar.alt = `Avatar de ${review.usuario.nombre}`;
              userAvatar.style.display = "block";
              if (userInitials) userInitials.style.display = "none";
            } else if (userInitials) {
              // Si no hay avatar, mostrar iniciales y ocultar imagen
              userInitials.textContent = review.usuario.nombre.charAt(0);
              if (userAvatar) userAvatar.style.display = "none";
            }
            card.querySelector(".user-name").textContent =
              review.usuario.nombre;
            card.querySelector(".review-rating").innerHTML = createStarRating(
              review.calificacion,
            );
