document.addEventListener("DOMContentLoaded", function () {
    ...
    function createOptimizedProductCard(producto, delayIndex) {
        ...
        const isFavorite = window.favoritesManager?.favoriteProducts.has(String(producto.id)) || false;
        ...
        card.querySelector('.product-brand').textContent = producto.marca || "Sin marca";
        card.querySelector('.product-name').textContent = producto.nombre;
        card.querySelector('.product-description').textContent = producto.descripcion.substring(0, 70) + (producto.descripcion.length > 70 ? "..." : "");
        card.querySelector('.product-price').textContent = `$${parseFloat(producto.precio).toFixed(2)}`;
        card.querySelector('.product-rating-stars').innerHTML = generateOptimizedStars(producto.calificacion_promedio || 0);
        card.querySelector('.product-rating-reviews').textContent = `(${producto.reseñas || 0} reseñas)`;
